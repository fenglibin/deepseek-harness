 ...
 * into the Chinese `.zh.md` page, rewriting subsystem-page type links to `.zh.md`
...
export const SERVICE_PAGE: Record<string, string> = {
  ...
  lightweightModel: 'llm-streaming.md',
...
export const LINK_MAP: Readonly<Record<string, string>> = {
  ...
  LightweightModelSelection: 'llm-streaming.md',
  LlmModelDiscoveryRequest: 'llm-streaming.md',
  LlmDiscoveredModel: 'llm-streaming.md',
  LlmConnectionCheckRequest: 'llm-streaming.md',
  LlmConnectionCheckResult: 'llm-streaming.md',
  LlmConnectionCheckOperation: 'llm-streaming.md',
...
/**
 * Rewrite `.md` type links in a generated region to their `.zh.md` Chinese
 * counterparts. The Chinese page is the only published side, so a link must
 * not point at the deleted English source — including the `../` form the
 * catalog footer uses and any `#anchor` it carries. A target with no Chinese
 * page (a deliberately English-only document) keeps its original link.
 * @param region - Complete marker-delimited region text.
 * @returns The region with `.md` type links rewritten to `.zh.md`.
 */
export function localizeRegionLinks(region: string): string {
  return region.replace(/\]\((\.\.\/)?([a-z-]+)\.md(#[^)]*)?\)/g, (link, up: string | undefined, name: string, hash: string | undefined) => {
    // Region links are written for a `docs/subsystems/` page, so a bare name
    // is a sibling subsystem page and `../` reaches the docs root.
    const dir = up === undefined ? SUBSYSTEMS_DIR : 'docs'
    if (!existsSync(resolve(root, dir, `${name}.zh.md`))) return link
    return `](${up ?? ''}${name}.zh.md${hash ?? ''})`
  })
}
...
export function computeOutputs(): [string, string][] {
  ...
  for (const page of pages) {
    ...
    const rel = `${SUBSYSTEMS_DIR}/${page.replace(/\.md$/, '.zh.md')}`
    let current: string
    try {
    ...
    try {
      outputs.push([rel, spliceRegion(current, localizeRegionLinks(region))])
    } catch (error) {
