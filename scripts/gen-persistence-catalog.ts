...
const OUT = 'docs/persistence-catalog.zh.md'
...
const LINK_MAP: Record<string, string> = {
  ToolCallId: 'subsystems/core.zh.md',
  ContentBlock: 'subsystems/core.zh.md',
  MessageSource: 'subsystems/core.zh.md',
  ScheduleChange: 'subsystems/schedule.zh.md',
  StreamChunk: 'subsystems/llm-streaming.zh.md',
  TokenUsage: 'subsystems/llm-streaming.zh.md',
  TodoItem: 'subsystems/todo.zh.md',
  TurnTrigger: 'subsystems/session.zh.md',
  TurnEndReason: 'subsystems/session.zh.md',
  SessionTitleEventData: 'subsystems/session-title.zh.md',
  SessionTitleLlmRequestEventData: 'subsystems/session-title.zh.md',
  SessionTitleModelProvenance: 'subsystems/session-title.zh.md',
  SessionTitleProviderId: 'subsystems/session-title.zh.md',
  SessionTitleSource: 'subsystems/session-title.zh.md',
  TeamId: 'subsystems/agent-team.zh.md',
  TeamMemberSnapshot: 'subsystems/agent-team.zh.md',
  TeamMessageId: 'subsystems/agent-team.zh.md',
  TeamMessageSnapshot: 'subsystems/agent-team.zh.md',
  TeamTaskSnapshot: 'subsystems/agent-team.zh.md',
}
...
function renderEvent(e: AnnotatedLogEventEntry): string[] {
  ...
  out.push(`来源：[\`${e.source}\`](../${e.source.split(':')[0]})`, '')
  return out
}
...
export function render(events: AnnotatedLogEventEntry[], envelopeTypes: EventEnvelopeTypeEntry[]): string {
  const lines: string[] = [
    '<!-- 由 scripts/gen-persistence-catalog.ts 生成——请勿手工编辑。',
    '     运行 `pnpm run gen-persistence-catalog` 重新生成。 -->',
    '',
    '# 会话持久化事件目录',
    '',
    ...
    '```',
    '',
    `来源：${envelopeTypes.map(entry => `[\`${entry.source}\`](../${entry.source.split(':')[0]})`).join(' · ')}`,
    '',
    '## 事件',
    '',
  ]
