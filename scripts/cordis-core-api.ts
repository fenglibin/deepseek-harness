...
export const CORDIS_CORE_API_PAGES: CordisCoreApiPage[] = [
  {
    out: 'docs/cordis-api/context.zh.md',
    title: '上下文',
    intro: '上下文是 Cordis 的核心对象：所有服务、事件和生命周期 API 都通过 `ctx` 访问。事件方法见[事件](events.zh.md)，副作用与当前 fiber 见 [Fiber](fiber.zh.md)，插件加载见[注册表](registry.zh.md)。',
    sections: [
      { kind: 'class', file: 'vendor/cordis/src/context.ts', symbol: 'Context', prefix: 'ctx.' },
      { kind: 'context-merge', file: 'vendor/cordis/src/reflect.ts', heading: '服务存储与混入' },
    ],
  },
  {
    out: 'docs/cordis-api/events.zh.md',
    title: '事件',
    intro: '每个上下文中都混入了事件分发 API。Harness 事件声明及其分发模式会生成到各自所属的[子系统页面](../subsystems/core.zh.md)。',
    sections: [
  ...
  {
    out: 'docs/cordis-api/fiber.zh.md',
    title: 'Fiber',
    intro: 'fiber 是一个已加载的插件实例，包含其生命周期状态、经过校验的配置以及已注册的作用。`ctx.fiber` 是当前 fiber，`ctx.effect()` 会将调用委托给它。',
    sections: [
      { kind: 'context-merge', file: 'vendor/cordis/src/fiber.ts' },
      { kind: 'class', file: 'vendor/cordis/src/fiber.ts', symbol: 'Fiber', heading: 'Fiber 类' },
  ...
  {
    out: 'docs/cordis-api/registry.zh.md',
    title: '注册表',
    intro: '插件加载与依赖注入。',
    sections: [
  ...
  {
    out: 'docs/cordis-api/service.zh.md',
    title: 'Service',
    intro: '上下文服务的基类。以插件形式加载的子类会将自身注册为 `ctx.<name>`。',
    sections: [
...
function sourceLink(source: string): string {
  ...
  return `[来源](../../${file}${line === undefined ? '' : `#L${line}`})`
}
...
function renderMember(prefix: string, member: MemberDoc): string[] {
  ...
  if (member.returns !== null && member.returns !== '') lines.push(`**返回** ${unlink(member.returns)}`, '')
