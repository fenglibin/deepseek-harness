...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/util/http-proxy': { kind: 'none', reason: '进程级传输策略；只改变字节如何抵达网络，不注册任何提示词、schema 或结果文本。' },
  'packages/util/lazy-require': { kind: 'none', reason: 'Host 侧模块加载原语；只决定依赖何时初始化，不注册任何面向模型的内容。' },
  ...
  'packages/core/lightweight-model': { kind: 'indirect', reason: 'The service supplies a provider/model route to auxiliary callers; those callers own the model-visible request and the result never enters the conversation history.' },
  ...
  'packages/client/ui-file-browser': { kind: 'none', reason: 'Browser-side UI plugin layer; registers nothing model-facing.' },
  ...
  'packages/llm/llm-failover': { kind: 'indirect', reason: 'The plugin rewrites the provider/model route of retried requests; request assembly and adapters own the model-visible content.' },
  'packages/llm/llm-round-robin': { kind: 'indirect', reason: 'The plugin rewrites the provider/model route of each step\'s request; request assembly and adapters own the model-visible content.' },
  ...
  'packages/fs/file-changes': { kind: 'none', reason: 'The changedFiles unit folds already-logged tool calls and results into a client-facing read model and registers nothing model-facing.' },
  'packages/fs/session-file-revisions': { kind: 'none', reason: 'The capture listener reads already-logged tool results and folds them into a client-facing read model; it registers no prompt, schema, or result of its own.' },
  'packages/api/session-file-revisions': { kind: 'none', reason: 'The Remote namespace exposes already-recorded revisions to the browser and registers nothing model-facing.' },
  ...
  'packages/api/file-browser': { kind: 'none', reason: 'The Remote and its bytes route serve the browser file manager; they register no prompt, tool, or session event.' },
