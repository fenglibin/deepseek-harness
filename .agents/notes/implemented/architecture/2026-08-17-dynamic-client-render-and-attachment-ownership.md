...
`ui-conversation` declares `conversation.message.images` and supplies attachment data, callbacks, authorized image loading, and its locale seat. `ui-attachment` waits on that declaration through `ctx.slots.inject()` and registers the historical image gallery/lightbox. The React implementations remain internal package values; cross-plugin composition uses slots. This package integration supersedes the direct-import ruling in the [attachment display note](../feature/2026-08-11-web-attachment-display-alignment.md) without changing that note's visual and interaction decisions. The former `conversation.input.attachments` slot and its draft rail/drop target were retired by the [inline-image composer note](../feature/2026-09-05-inline-image-composer.md); draft images are now inline editor chips owned by `ui-conversation`, and `ui-attachment` renders only transcript history images.
