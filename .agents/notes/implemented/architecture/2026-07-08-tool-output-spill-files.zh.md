...
**使用 `ctx.fs.writeText` 或面向模型的 `write` 工具。** 不予采纳：文件浏览器系统写入带有项目文件语义、写入／编辑策略、观察状态和面向用户的副作用。spill 文件是运行时产物，不是由模型编写的工作区改动。现有 `read` 工具之后可以检查它们，但创建操作属于运行时 spill seam。
