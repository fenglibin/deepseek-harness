...
`agent/pre-step` 在无当前任务（或当前是 `l0`）且收到直接人类请求时发起一次分级调用，拿到 tier 后再建任务——因此任务一开始就是最终级别，不需要事后提升。

判定**不阻塞步骤**：`pre-step` 立即 `next()`，判定作为后台 pass 推进，落地时才建任务并补记依据。`user/message` 只在 `pre-step` waterfall 解析之后 append，所以在步骤里 `await` 判定会让用户自己发出的消息在整个判定期间不进入 transcript（实测 6.9 秒）。`l1`/`l2` 任务既不参与模型可见内容，也不应决定消息何时可见。

每个 agent 至多一个 pass 在跑，冲突的第二次请求被记为待续而非丢弃：两个 pass 会争抢同一个任务槽，而丢下它会让连续发送的第二次请求逃过纪律。pass 落地时重新读取当前任务，因此模型在此期间自建的任务不会被覆盖。
...
- **代价** 每个 ≤200 字的直接请求多一次模型调用。该调用与步骤并行，因此不为用户消息增加等待，但任务的出现会晚于消息本身。`l1`/`l2` 任务存续期间不再重复调用。
