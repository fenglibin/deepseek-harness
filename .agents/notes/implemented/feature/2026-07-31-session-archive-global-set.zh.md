...
归档后 UI 的恢复面后来由 [已归档会话设置页](2026-09-12-session-unarchive-settings-page.zh.md)补齐，其入口是 `workspace.unarchiveSession`（逆向 RPC）加一个设置分节，本记录预测的两项都已交付，且归档集合的存储、注册表与全快照姿态未变；数据与 slot 完好。`workspace.list` 响应形状变化是 pre-release 直改（无兼容层）。e2e（workspace-management）钉住了「归档→行消失→reload 后仍隐藏、日志仍在」的全链路；domain 层测试钉住幂等、未知 id 拒绝、跨重启恢复与旧介质默认升级。
