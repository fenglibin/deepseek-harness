...
**连思考语言也强制。** 否决：思考内容是模型自有的；harness 注入的是系统提示段，任何措辞都无法可靠地规定内部思考语言。加强后的指令只管用户可见的正文，那才是用户读到的内容。该否决随后被[思考过程跟随响应语言指令](2026-09-14-reasoning-follows-response-language.zh.md)部分取代：那条指令接受可靠性有限这一前提，只改变英文推理出现的比例。
