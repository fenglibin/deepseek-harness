...
**A Bash row's output height is a stage, carried as `data-stage` on a wrapper.** The row carries its stage as `data-stage` on a wrapper, and its cap derives from `--dsl-terminal-line-height` (`calc(var(--dsl-terminal-line-height) * 10)`) rather than repeating a pixel height that would drift from the font binding. `maxLines` stays `Infinity` throughout: the primitive's own `maxLines` collapses the *middle* of a long output, which is a different gesture from the scroll the row wants.

As shipped here the row had three stages — a two-line `peek` on the closed row, `full` at ten lines when open, and `all` past the cap — and rendered the terminal card at every one. [One lifecycle contract for tool-row disclosure](../bug-fix/2026-09-04-web-row-lifecycle-disclosure.md) later removed `peek`: a settled bash row now collapses to its one-line summary like every other tool row, and the card mounts and unmounts with `open`. The ten-line cap, the `all` stage, and the control below are unchanged.
...
**Render the terminal only when open (the previous behavior), changing just the cap.** Rejected here: it leaves the closed row unable to answer "what did it print", which is the question a command row is most often asked. Showing two lines costs nothing the reader has to open. This rejection was reversed two days later — a fold that still shows the output box does not read as a fold, which is the report [one lifecycle contract for tool-row disclosure](../bug-fix/2026-09-04-web-row-lifecycle-disclosure.md) records.
...
Two existing tests encoded the old Bash contract — that the terminal card is absent until the row is opened — and were updated to assert the stage attribute instead, since the card was then present at every stage and only its height changed. The same is true of the `assembly-surfaces` coverage of the keyed row reaching its terminal card. Since `peek` was later removed those tests assert the card's absence again; see [one lifecycle contract for tool-row disclosure](../bug-fix/2026-09-04-web-row-lifecycle-disclosure.md).
...
The ten-line cap is CSS text, which jsdom cannot lay out, so `chat-font-axis-styles.client.spec.ts` and `tool-row-styles.client.spec.ts` assert the declarations — including that the cap derives from `--dsl-terminal-line-height` rather than restating a pixel height. The stage machine itself is pinned in `terminal-card.client.spec.tsx`, which walks `full -> all` and back.

Showing the terminal on the closed row cost DOM that it previously did not: every output line was mounted at every stage, where a closed row used to render one summary line. The cap was visual — `overflow` hid the excess without removing it from the tree — so the cost was bounded by the largest output in the session rather than by what the reader had opened. Removing `peek` retired that cost with it; see [one lifecycle contract for tool-row disclosure](../bug-fix/2026-09-04-web-row-lifecycle-disclosure.md).
