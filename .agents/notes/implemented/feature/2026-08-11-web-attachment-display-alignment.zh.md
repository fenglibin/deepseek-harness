...
附件展示位于 `@deepseek-ai/dsh-client-ui-attachment`（`packages/client/ui-attachment`）：`MessageImage`/`ImageGallery`（单击预览），以及 `ImageLightbox`。这些组件仍是包内的纯 props 组件。`ui-conversation` 声明消息图片 slot，并提供图片加载、回调及其 locale seat；动态 ui-attachment 客户端 entry 等待该声明并注册呈现。[动态渲染与附件归属 Note](../architecture/2026-08-17-dynamic-client-render-and-attachment-ownership.zh.md)负责这项包集成决策；本 Note 记录的视觉行为保持不变。原先的 `AttachmentRail` 及其拖放目标已由[内联图片 composer Note](2026-09-05-inline-image-composer.zh.md)退役：草稿图片现在是 `ui-conversation` 持有的内联编辑器 chip，`ui-attachment` 只渲染 transcript 历史图片。
...
历史图片界面的交互模型与 DeepSeek Chat 一致，纯 props 组件通过 conversation slot 的 locale seat 渲染，无需触达应用状态。代价是一个真实的动态包边界：`ui-attachment` 带有标准插件脚手架（客户端 bundle、invariant 伴生、双语 README、tsconfig face、逐文件 100% 覆盖率），省略该插件会让唯一可选的消息图片 slot 保持为空。错误横幅是短时的——用户移开视线四秒就会错过消息，这正是 DeepSeek Chat 自己做的取舍。非图片附件仍不支持；输入框的接收仍只认图片（记录于包 README 的限制一节）。
