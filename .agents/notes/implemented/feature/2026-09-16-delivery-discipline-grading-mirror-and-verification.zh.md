...
**保持 `todo` 与 `delivery-tasks` 双源，仅改 UI 分别展示。** 否决：漂移问题依旧，只是从「看不见」变成「看得见的两个互相矛盾的列表」。宿主 fold 的镜像给出唯一源之后，两个面板改读同一份值（[单一数据源](../bug-fix/2026-09-17-task-progress-single-source.zh.md)）。
