...
- 粘贴或拖放一张或多张受支持的图片后，光标处会按顺序插入内联 chip，但不会插入占位文本。文件拖入输入区时会高亮放置目标。
- 草稿图片渲染为内联编辑器 chip（每张图片一个 decorator 节点），而不是独立附件栏。原先的 `InputBar` 附件栏——文本框上方的缩略图、悬停显示的移除按钮、单击预览与两端箭头翻页——已由[内联图片 composer Note](2026-09-05-inline-image-composer.zh.md)退役；其间的视觉细节记录于[附件展示对齐 Note](2026-08-11-web-attachment-display-alignment.zh.md)。
- 提示词可同时包含文本与图片，也可仅包含图片。粘贴纯文本时保持浏览器原生行为；粘贴混合的剪贴板内容时，文本会正常插入，文件则同时作为内联 chip 添加；仅粘贴文件时才阻止浏览器的默认处理。在输入区放置文件时总会阻止浏览器导航，并在本地报告不受支持的文件。
...
| `packages/client/ui-conversation` | 每个会话的草稿图片（内联编辑器 chip）、用户与助手图片控件和原图预览。 |
