...
> 范围：完成的轮次以其产出文件收尾的那一行、读得出是链接的文件路径链接，以及 Host 打开器对浏览器可渲染文档优先选用默认浏览器。当时的范围判断把「以 HTTP 提供文件浏览器」与「为不在 Host 机器上的客户端提供预览」排除在外；这两条已被[会话文件链接的只读查看](../architecture/2026-09-16-session-file-readonly-viewer.zh.md)取代——会话文件链接现在在浏览器内只读呈现，桌面打开器降为查看器内的次要入口。本 note 仍然拥有的是产出文件行、链接可发现性，以及对「把收尾消息链接化」的否决。
...
**以 HTTP 提供文档不在范围内。** 由 harness 自己提供文件——与 `/api` 同源、置于 `CSP: sandbox` 之后、或交给一个以自身端口给所服务文档独立源的第二监听器——随产品范围一并否决。这条判断针对的是**提供任意文档**：所否决的三条替代方案在下面记录了它们各自的失败方式（同源不可隔离、`CSP: sandbox` 会杀死所服务页面）。它不约束[会话文件链接的只读查看](../architecture/2026-09-16-session-file-readonly-viewer.zh.md)：那条路径不提供文档，而是把文本读进产品自己的查看器，并且由 Host 做工作区包含检查。
