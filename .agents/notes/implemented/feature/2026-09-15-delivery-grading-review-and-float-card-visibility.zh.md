...
- **显隐偏好用 `defineStore` + `persist`。** 新增 `createDeliveryCardStore()`：`init` 为 `{ visible: false }`，持久化键 `dsh.delivery.float-card`，root 作用域，因此所有会话共享同一偏好。组件常驻挂载，在 `document` 上监听 `Ctrl+Shift+P` 并 `preventDefault` 后调 `actions.toggle()`；隐藏时返回 `null`。
- **任务列表回落到 `todos` 投影。** 「任务列表」组在 `delivery-tasks` 清单为空且 `todos` 非空时渲染 todo 项与三态状态，并在组头标注「来自当轮清单」。交付清单非空时 `todos` 完全不参与渲染，两套来源不混排。
...
**合并交付清单与 `todos` 两套来源。** 否决：同一任务可能两处都有，去重规则主观，反而制造新的不一致。

## 后果

- **获得** 分级判定在判断边界上交给模型，只有确定性的大改动才自动进入交付流程；设计文档可从卡片直接定位；进度栏默认不遮挡正文且偏好跨刷新保留；走 `todo_write` 的执行在进度栏上可见；非 `l2` 任务也有了落盘清单的指引。
- **代价** `l1` 请求每轮多一次约 100 token 的 rubric 注入，且模型可能选择不建任务——这是「按内容判断」本身的代价；`l0`/`l1` 自动建任务的旧快照与 `l1` 自动建任务的用例需随之改写；客户端新增一个 localStorage 键 `dsh.delivery.float-card`。
- **延后** 需求的后续调整不会让已记录的清单自动失效，界面展示的仍是最后一次 `record_tasks` 的清单；`todos` 回落只在当轮可见，因为该投影在 `turn/start` 清空。

## Verification

`packages/delivery/tool-delivery/tests/tool-delivery.spec.ts` 的 auto-detect 组固定「中信号请求不自动建任务且注入 rubric」与「强信号仍自动建 `l2`」。`packages/client/ui-delivery/tests/delivery-float-card.client.spec.tsx` 覆盖默认隐藏、其它按键组合不触发、快捷键展示与隐藏、偏好写入 localStorage、卸载后不再监听、设计文档链接渲染与点击转交 opener、打开失败就地提示、清单为空时回落 `todos` 且清单非空时不混排。`browser-plugin.client.spec.tsx` 经注册入口验证 opener 把路径原样交给 `session.openWorkspacePath`，并把它拒绝的结果变成 rejected promise。
