...
- **生命周期**（顶层文件夹）是 Agent Note 的状态，Agent Note 随状态变化在文件夹之间移动：
  ...
  - **`rejected/`**：提案经过讨论后被否决。仅当其决策依据仍能避免一种诱人且影响重大的错误时保留；否则删除其记录文件。
...
当一份 implemented Agent Note 记录的交付决策已经完整落地，且其决策依据不太可能再指导未来工作时，将其归档。如果其中的备选方案、归属边界、否定性保证、持久化语义或协议语义、安全规则，或者重新引入条件仍有价值，则继续作为活跃记录保留。绝不归档 proposed Agent Note：过时的提案应转为 rejected。仅当 rejected Agent Note 仍能避免一种可能发生的错误时保留；否则删除其记录文件。请使用经过校准的 [`dsh-archive-agent-notes`](../skills/dsh-archive-agent-notes/SKILL.md) 工作流，不要根据字数、存续时间或目标配额来判断。

归档路径编码为 `archived/{class}/yyyy-mm-dd-topic-title.zh.md`；其中有意省略 `implemented`，因为只有 implemented Agent Note 可以进入归档。仓库的 Agent Note 是中文单语的，归档变更会移动该 `.zh.md` 文件，保留 `Status: implemented`，紧接该状态行插入 `Archived: YYYY-MM-DD` 行，并修复或删除入站链接。归档时只允许对内容做这些更改。

封存后，每份归档文件都永久冻结。禁止编辑、翻译、重新格式化、更新、移动或删除，也不得将其视为当前行为的权威依据。文档门禁会跳过归档源文件，包括其中的出站链接；当活跃文档有意引用历史时，仍可链接到归档 Agent Note。[`verify-archived-agent-notes`](../../scripts/verify-archived-agent-notes.ts) 强制执行封闭的类别目录树、中文归档元数据，以及仅追加的冻结内容 manifest（元数据清单）。[归档政策 Agent Note](implemented/process/2026-07-26-frozen-agent-note-archive.zh.md) 记录了设计依据。
...
每份 Agent Note 的正文以问题陈述开头：动机，写法上不依赖解决方案即可独立成文。后续内容取决于生命周期；固定章节使用以下规范名称且仅限这些名称，而真正独特的技术章节（包拓扑、协议约定、schema 等）在必需章节之间可自由组织。章节标题可用英文规范名或其中文对应（门禁按中英同义词识别）；新写 note 建议沿用英文规范名，历史中文 note 保留其既有中文标题即可。Agent Note 是中文单语文件（`.zh.md`），不存在对侧翻译。
