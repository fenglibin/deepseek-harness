...
1. Move the `foo.zh.md` file from `implemented/<kind>/` to `archived/<kind>/`; `implemented` is deliberately absent from the archive path. Agent Notes are Chinese-only; there is no English side or sidecar to move.
2. Make no body edits. Insert only `Archived: YYYY-MM-DD` immediately below `Status: implemented`.
3. Do not translate, reformat, update facts, or repair links inside the note.
...
5. Run `pnpm run verify-archived-agent-notes --write`. Its append-only mode first proves every existing seal still matches, then adds only the new note hash. Run the normal verifier afterward.
