...
默认值 `zh` 不读任何信号：无论宿主区域设置如何，模型都用中文回答。想跟随环境的部署把 `language` 设为 `auto`，它先读取 Web 界面已保存的语言选择，再读取宿主进程自身的区域设置，并在两者都没有指向本 row 能指导的语言时不注册任何段落。该指令是提示词**段落**而非运行时上下文，所以它能穿过 `includeRuntimeContext: false`，并且能到达子智能体——子智能体的装配会合并全局层。
...
|---|---|---|
| `language` | `'zh'` | `zh` 固定中文，不读任何信号；`auto` 依次遵循界面语言与宿主区域设置；`en` 固定英文；`off` 完全不注册段落 |

生成的[配置目录](../../../docs/config-catalog.zh.md#deepseek-aidsh-response-language)是每个可接受字段及其 JSDoc 的完整来源。

### 哪个信号优先

`zh` 与 `en` 是钉死值，不参与解析。只有 `auto` 按以下顺序解析，并在遇到第一个能指名语言的信号时停止。没有已发布指令的语言同样会终止搜索，而不会落到后面的信号上，因此中文宿主上的英文界面选择会生效并产生「无段落」：
...
因为英文是模型无需指令就会使用的语言，`en` 与 `off` 都不输出内容；两者的意图不同：`en` 是部署方记录下来的固定选择，表示无论宿主如何都要英文。`off` 是在需要抑制该段落时唯一的方式，例如面向非中文读者的部署。
...
只有一个段落，且仅在解析出的语言拥有已发布指令时出现。它要求该语言用于人会读到的全部内容，包括模型的思考过程与模型自己撰写的代码注释、文档注释，同时保留模型不得改写的代码标记，并让模型未撰写的既有注释维持原语言。
...
固定开销：约 200 个 token 的一段文本（约 320 字），在整个会话中要么存在要么不存在，不随对话增长。
...
- **思考语言同样只是提示词层面的软约束** —— 思考通道由模型自有，指令只降低英文推理出现的概率；模型在长工具链上仍可能回到英文推理。
