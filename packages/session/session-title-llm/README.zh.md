...
`provider` 与 `model` 覆盖项都是可选的，但必须同时作为非空字符串提供。如果没有这一对取值，辅助函数使用[轻量模型](../../core/lightweight-model/README.zh.md)选中的路由；未选中任何路由时，回落到当前会话已记录 `request/header` 中捕获的确切提供方／模型路由，因此在任何路由出现前显式刷新时必须提供覆盖项。辅助函数在记录或分发前，依据 `maxInputBytes` 检查最终 JSON 封装用户提示词的大小，而不是将其截断，并在消费流期间与完成后重新检查超时与调用方取消，因此即使拦截器或适配器忽略 abort，也不能接受迟到的成功结果。格式错误或空输出、工具调用与非 stop 结束原因都会拒绝；会话标题服务决定该拒绝属于自动警告还是显式调用方失败。
...
- **思考 token 计入 `maxOutputTokens`**——会思考的路由会先把整个预算花在思考上，之后才产出标题，因此触达 `maxOutputTokens` 的思考型路由会返回空正文，调用方于是保留兜底标题。请按该路由实测的思考开销来设置 `maxOutputTokens`，或用[轻量模型](../../core/lightweight-model/README.zh.md)把这次调用指到别处。
