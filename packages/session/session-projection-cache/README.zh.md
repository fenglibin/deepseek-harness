...
两个节流字段均必填——写入节奏是部署选择，没有普适正确值；两个保留字段有默认值，因为它们约束的是本服务自身的常驻内存：
...
| `maxRowBytes` | 4 MiB | 值得缓存的最大检查点文档（估算 JSON 字节）；更大的行不写入，已存储的会被丢弃 |
| `budgetBytes` | 64 MiB | 常驻检查点总字节预算；超预算时按最冷优先丢弃，仍被挂载的会话的行永不作为牺牲者 |
...
`per-record` 布局在打开域时把每条已存记录读入内存，因此缓存的内存若无约束就随会话数量线性增长。两条保留策略把它变成预算：超过 `maxRowBytes` 的检查点不写入（已存储的会被丢弃），常驻总量超过 `budgetBytes` 时按最冷优先丢弃记录，启动时先做一次同样的清扫。丢弃的是派生数据——下一次冷读重新折叠日志即可，代价只是一段更长的尾部重放。仍被挂载的会话的行不会被选中，所以当所有行都属于活跃会话时预算是软上限。

### 读取缓存值

`cachedSnapshot(meta)` 以零 I/O 从存储域的内存表同步提供客户端值。它只接受身份匹配的记录以及版本和 schema 均匹配的 key，再按所服务行的最低水位返回 `{ asOfSeq, values }` 切面。对于未知 id、无关生命周期、缺失或外来的记录文档，或没有可用行的情况，它返回 `undefined`。`coldSnapshot(meta, events)` 接受完整有序日志，在折叠时跳过已检查点化的前缀，并在自身不读取持久化层的情况下刷新记录。`remove(id)` 丢弃一个会话的缓存行并报告是否存在该行：这些行派生自本包自身从不读取的日志，因此删除该日志时必须一并删除它们——此后的任何读取都无法区分一个重建的 id 与一个继承了这些行的 id。
...
- **淘汰按字节预算，不按时间**——行按常驻字节总量与最冷优先被丢弃，没有按龄或按会话数的保留窗口；一次超预算的淘汰可能丢掉某个刚变冷会话的行，它的下一次冷读需要重折叠日志。
- **同一会话的两个切面可以乱序落盘**——领域写入链是 last-write-wins，而较早取得的切面可能较晚入队（例如创建写入先等待日志落盘，而释放写入不等）。此时缓存会停在较旧的水位上，直到下一次检查点把它推新；行始终不会错，只会更陈旧。
