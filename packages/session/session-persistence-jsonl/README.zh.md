...
- **列举成本随会话数线性增长**——`list()` 与 `listSnapshots()` 遍历根下的每个会话目录并读取每份日志的 header 行：没有摘要索引，也没有跨调用缓存，因此每次会话列表刷新都会重新付这笔成本（本地 SSD 实测约 230µs/会话，5000 会话约 1.1s），而成本不随日志大小增长。会话数达到数千量级且列表刷新频繁时，改用 [SQLite 后端](../session-persistence-sqlite/README.zh.md)——它的元数据落在 `sessions` 表上，列举不触碰事件行。
