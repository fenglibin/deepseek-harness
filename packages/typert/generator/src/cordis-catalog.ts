...
const GATE_NOTICE = '本文档根据源码生成（`scripts/gen-cordis-catalog.ts`），并由 `pnpm run verify-cordis-catalog`（`doc-sync`（文档同步门禁）的一部分）验证新鲜度——请勿手工编辑。签名块使用 `ts cordis-catalog` 围栏，并在每个事件或服务方法前保留原始源码 JSDoc。doc-typecheck 会跳过这些裸声明片段；签名中的类型名称会链接到记录该类型的页面。'
...
export function renderPageRegion(page: string, services: ServiceEntry[], events: EventEntry[], policy: CordisCatalogPolicy): string {
  const lines: string[] = [
    ...
    '## Cordis API',
    '',
    'Generated from source by `scripts/gen-cordis-catalog.ts` (verified fresh by `pnpm run verify-cordis-catalog` in doc-sync; regenerate with `pnpm run gen-cordis-catalog`) — the language sides differ only in locale-specific paired document paths. Signature blocks use a `ts cordis-catalog` fence and keep the original source JSDoc; dispatch modes are defined in the [primer](../cordis-primer.md#dispatch-modes), and the framework-inherited `ctx` API lives in [cordis-api/inherited.zh.md](../cordis-api/inherited.zh.md).',
    '',
  ]
