...
- **No efficiency hints** — the bash tool appends `[hint: …]` lines when a command matches a pattern that names Unix commands (`grep`, `find`, `sleep`) or a `pnpm`/`vitest` run; none exist on the PowerShell path, so this twin renders result text without them ([runtime-hints Agent Note](../../../.agents/notes/implemented/feature/2026-09-04-bash-result-efficiency-hints.md)).
