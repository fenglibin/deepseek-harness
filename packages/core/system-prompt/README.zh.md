...
段默认参与 `{{variable}}` 插值。外部文本（例如 MCP 服务器指令）可能含花括号，按变量解析会抛未知变量错误，因此注册时可以声明 `interpolate: false`：该段的文本作为字面量进入组装结果，其中的花括号保持原样。字段缺省按 `true` 处理，既有段行为不变。该字段同时出现在 `PromptSection`（注册输入）与 `AssembledSection`（组装结果）上，`renderPrompt` 按它分支。
