...
`deleteSession` is the only operation that removes a Session: it retires any still-attached ordinary Agent (taking its Session out of the store and draining whatever it still owes durable storage), then discards the durable log through `ctx.sessionPersistence.remove`, drops the derived projection-cache row through `ctx.sessionProjectionCache.remove` when that optional service is mounted, drops the Host's references through `ctx.workspaceRegistry.removeSession`, and emits `api-session/removed` so every connected Client drops the row. Deleting is not archiving — an archived Session keeps its log and its place in its Workspace rows, while a deleted one has neither. `deleteSession` refuses with `RemoteError('session/live', …)` only while the Session's Agent is still running a turn; merely having opened a Session does not make it running. An attached Agent this controller cannot retire — one configured, subagent-owned, or created elsewhere — is refused the same way. A deployment that mounts no persistence backend refuses the delete with `gateway/internal`. An absent log reports `session/not-found`; any other persistence failure reports `gateway/internal` and removes nothing.
