...
describe('WorkspaceController', () => {
  ...
  it('maps generated business failures to the command facade errors', async () => {
    ...
    remote.unarchiveSession.mockResolvedValueOnce(remoteFailure(
      new RemoteError('gateway/internal', 'restore write failed', {}),
    ))
    await expect(controller.unarchiveSession(sid('session')))
      .rejects.toThrow('workspace session restore failed: gateway/internal: restore write failed')
