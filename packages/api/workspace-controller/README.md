...
`@deepseek-ai/dsh-api-workspace-controller` owns the Host `ctx.workspaceController` service and the generated Client `ctx.remote.workspace` namespace. Its Remote methods create, rename, remove, and reorder Workspaces, reorder Sessions within a Workspace, archive and unarchive Sessions in Workspace navigation, and follow the complete Workspace projection. Use it through API Gateway when a Client must change or follow Workspace navigation. The package also owns `ctx.directoryPickerController` and the generated `ctx.remote.directoryPicker` namespace, because the directory-picking seam it carries is abstract and never a Loader entry of its own.
...
The archive set is display membership, not ownership. `archiveSession` adds one known Session to the registry-global set — a Session the registry cannot find fails with `session/not-found` — while the Session keeps its slot in its Workspace rows, so `unarchiveSession` restores it to the position that slot kept. Both verbs return the complete resulting archive set; the Client `workspace` service exposes both as `Promise<void>`, and `ClientWorkspaceModel` installs the returned set into its rows. Unarchiving an id outside the archive set resolves without writing, so a Session restored twice — or one that was never archived — is not an error.
