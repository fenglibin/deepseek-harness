...
内容采样取开头 `SNIFF_BYTES`（8192）字节，并按采样是否**截断**区分两种尾部不完整序列：采样只是文件前缀时，切分点可能落在某个多字节字符中间，因此用流式解码容忍这段不完整序列——否则字节 8192 恰好切在 CJK 字符里就会把一份普通 UTF-8 文本报成二进制；采样就是整个文件时，尾部的不完整序列是真的截断内容而不是切分产物，因此严格解码把它报出来。含 NUL 字节仍然直接判为二进制。
