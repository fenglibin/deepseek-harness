...
`deriveTurnTokenUsage(events)` folds one complete Turn into exact per-attempt and total usage for browser consumers. Missing lifecycle evidence, unsafe counts, or contradictory exact totals return no result. The cache-read and cache-write aggregates sum the attempts that report the bucket — a silent attempt contributes zero, the convention `tokenUsage` also applies — and appear once any attempt reports them; the reasoning and route aggregates still require every contributing attempt.
