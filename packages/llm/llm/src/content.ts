...
import type { ContentBlock, ImageDescription } from './types.ts'
...
/** Leading hex of one attachment id, without the `sha256:` scheme prefix. */
function attachmentDigest(ref: ImageAttachmentRef): string {
  return String(ref.attachmentId).slice('sha256:'.length, 'sha256:'.length + 8)
}
...
export function textOnlyImageText(ref: ImageAttachmentRef): string {
  return `[image omitted because this model accepts text only; attachment sha256:${attachmentDigest(ref)}]`
}

/**
 * Stable text carrying one generated description for a model that cannot
 * accept the image itself. The describing model is named so the model can tell
 * this account of the image apart from the user's own words.
 * @param ref - durable normalized attachment the description was made from.
 * @param description - generated text and the route that produced it.
 * @returns deterministic text-only rendering of the description.
 */
export function describedImageText(ref: ImageAttachmentRef, description: ImageDescription): string {
  const digest = attachmentDigest(ref)
  return `[image description generated by ${description.source.model}; attachment sha256:${digest}: ${description.text}]`
}
...
/**
 * Collect every durable image reference in occurrence order, walking nested
 * tool-result content. This is the reference-ordered walk that pairs with
 * {@link attachImageDescriptions}, so one description batch answers every
 * occurrence in the same order.
 * @param blocks - typed model content blocks.
 * @returns one reference per image occurrence, in occurrence order.
 */
export function collectImageRefs(blocks: readonly ContentBlock[]): ImageAttachmentRef[] {
  const refs: ImageAttachmentRef[] = []
  collectImageRefsInto(blocks, refs)
  return refs
}

/** Append image references in nested-block order into a shared accumulator. */
function collectImageRefsInto(blocks: readonly ContentBlock[], refs: ImageAttachmentRef[]): void {
  for (const block of blocks) {
    if (block.type === 'image') refs.push(block.attachment)
    else if (block.type === 'tool-result') collectImageRefsInto(block.content, refs)
  }
}

/**
 * Attach generated descriptions to image blocks in occurrence order, walking
 * nested tool-result content, without mutating durable blocks. One description
 * entry pairs with one image occurrence; `undefined` leaves that occurrence
 * undescribed.
 * @param blocks - typed model content blocks.
 * @param descriptions - one description or `undefined` per image occurrence, aligned by occurrence order.
 * @returns the original blocks when nothing changes, otherwise shallow copies with descriptions attached.
 */
export function attachImageDescriptions(
  blocks: readonly ContentBlock[],
  descriptions: readonly (ImageDescription | undefined)[],
): ContentBlock[] {
  let cursor = 0
  const attach = (list: readonly ContentBlock[]): ContentBlock[] => {
    let next: ContentBlock[] | undefined
    for (const [index, block] of list.entries()) {
      if (block.type === 'image') {
        const description = descriptions[cursor++]
        if (description !== undefined && block.description !== description) {
          next ??= list.slice(0, index)
          next.push({ ...block, description })
          continue
        }
      } else if (block.type === 'tool-result') {
        const content = attach(block.content)
    ...
    return next ?? list as ContentBlock[]
  }
  return attach(blocks)
}
    ...
    if (block.type === 'image') {
      ...
      next.push({
        ...
        text: block.description === undefined
          ...
          : describedImageText(block.attachment, block.description),
      })
      continue
    }
    if (block.type === 'tool-result') {
