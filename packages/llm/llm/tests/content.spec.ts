...
describe('projectImagesForTextModel', () => {
  ...
  it('renders a generated description instead of the omission placeholder', () => {
    const described = {
      type: 'image' as const,
      attachment: image(3).attachment,
      description: {
        text: 'A red traffic light.',
        source: { provider: 'deepseek-official', model: 'deepseek-v4-flash-vision-exp', instruction: 'Describe it.' },
      },
    }
    const messages = [createUserMessage({ content: [{ type: 'text', text: 'lead' }, described], source })]

    expect(describedImageText(described.attachment, described.description)).toBe(
      '[image description generated by deepseek-v4-flash-vision-exp; attachment sha256:aaaaaaaa: A red traffic light.]',
    )
    expect(projectImagesForTextModel(messages)[0]?.content[1]).toEqual({
      type: 'text',
      text: '[image description generated by deepseek-v4-flash-vision-exp; attachment sha256:aaaaaaaa: A red traffic light.]',
    })
  })

  it('keeps the omission placeholder for an image with no description', () => {
    const messages = [createUserMessage({ content: [image(3)], source })]
    expect(projectImagesForTextModel(messages)[0]?.content).toEqual([
...
describe('collectImageRefs + attachImageDescriptions', () => {
  const description = (text: string) => ({
    text,
    source: { provider: 'p', model: 'm', instruction: 'i' },
  })

  it('collects direct and nested image references in occurrence order', () => {
    const direct = image(3)
    const nestedFirst = image(4)
    const nestedSecond = image(5)
    const tail = image(6)
    const nested = {
      type: 'tool-result' as const,
      toolCallId: ToolCallId('shot'),
      content: [nestedFirst, { type: 'text' as const, text: 'between' }, nestedSecond],
    }
    expect(collectImageRefs([direct, nested, tail])).toEqual([
      direct.attachment,
      nestedFirst.attachment,
      nestedSecond.attachment,
      tail.attachment,
    ])
  })

  it('returns no references for image-free content', () => {
    expect(collectImageRefs([{ type: 'text', text: 'plain' }])).toEqual([])
  })

  it('attaches descriptions by occurrence order without mutating durable blocks', () => {
    const direct = image(3)
    const nestedImage = image(4)
    const nested = {
      type: 'tool-result' as const,
      toolCallId: ToolCallId('shot'),
      content: [nestedImage],
    }
    const blocks = [direct, nested]
    const attached = attachImageDescriptions(blocks, [description('first'), description('second')])
    expect(attached).not.toBe(blocks)
    expect(attached[0]).toEqual({ ...direct, description: description('first') })
    expect(attached[1]).toEqual({ ...nested, content: [{ ...nestedImage, description: description('second') }] })
    expect(direct.description).toBeUndefined()
  })

  it('leaves an occurrence undescribed when its entry is undefined', () => {
    const blocks = [image(3), image(4)]
    const attached = attachImageDescriptions(blocks, [description('only'), undefined])
    expect(attached[0]).toEqual({ ...image(3), description: description('only') })
    expect(attached[1]).toEqual(image(4))
  })

  it('returns the original list when no description changes', () => {
    const plain = { type: 'text' as const, text: 'plain' }
    expect(attachImageDescriptions([plain], [])).toEqual([plain])
    const described = { ...image(3), description: description('same') }
    expect(attachImageDescriptions([described], [description('same')])).toEqual([described])
  })
})
