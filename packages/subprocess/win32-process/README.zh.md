...
- **唯一可复用 ABI owner** — `abi.ts` 拥有 sandbox process 路径消费的 Win32 常量与 x64 布局值。`ffi.ts` 懒加载 `kernel32.dll` 与 `advapi32.dll`，核验 `STARTUPINFOW` 和 `PROCESS_INFORMATION`，提供带类型的操作与错误格式化，并让 sandbox policy 通过同一组已加载库绑定剩余 API。绑定与 Koffi 类型都在首次原生操作时才解析，因此导入本包不加载 `koffi`，也不注册它的进程级类型名。
...
Koffi 的 `STARTUPINFOW` 与 `PROCESS_INFORMATION` 定义还会断言各自的 64 位大小；这两项断言随类型物化在首次原生操作时执行，由 `startupInfoType()` 与 `processInformationType()` 触发。其余已记录偏移和常量由该探针提供证据。
