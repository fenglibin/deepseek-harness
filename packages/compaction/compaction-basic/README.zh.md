...
更早压缩并不是免费的收益。压缩会替换历史，而替换会改写有序 surface，使下一次请求在该点与上一次请求的提示词前缀分叉，从而未命中整个对话的提供方 prefix cache——这还不包括每次压缩所需的那次摘要请求。在一次 75 步的实测会话中，提供方侧的失效只发生了 2 次，占总耗时的 4% 至 7%；把 `thresholdRatio` 调低会让这种偶发未命中变成每次压缩都必然发生。除非测量给出相反结论，否则保持 `0.8` 默认值。
