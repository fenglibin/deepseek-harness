...
- **`.env`**——你的普通环境层：调用目录的文件优先于 harness home 的文件，两者都低于继承环境。决定进程如何启动的变量（`PATH`、`DSH_*`、`XDG_*` 等）会被文件拒绝：请改为导出。harness home 的 `.env` 是唯一例外，它还可以声明 `HTTP_PROXY`、`HTTPS_PROXY`、`ALL_PROXY` 与 `NO_PROXY`——代理必须能在第一个插件挂载前从启动快照解析出来，而继承环境不含这些变量时只有该层能提供它们；调用目录的 `.env` 与项目层不享受这项豁免。对于只想加载某个目录 `.env` 的非产品 bin，文件缺失不影响启动，文件无法加载时输出一行带标签的警告。
