...
当 settings provider 是文件类型时，manager 解析出 `mcp.json` 的路径（settings 文档同目录），先播种缺失的文档，再用 chokidar 监听它的变化。每次变化走一条单向路径：`readFile` → `parseMcpJson`（JSON 语法与 `mcpServers` 结构校验）→ `mcpJsonToSettings`（`mcpServers` map 到 `servers` 数组，`disabled` 取反、`command`/`url` 判别传输）→ `SettingsScope.replace`。任一步失败都会被记录并跳过，绝不会把半成品写进 settings；转换本身是纯函数，集中在 `mcp-json.ts`。文档缺失时，`bootstrapMcpJson` 从当前 `scope.get()` 反向渲染一份 `mcp.json`，因此首次手动编辑从 settings 已有的内容开始。`writeMcpDocument`、`updateMcpServer` 与 `removeMcpServer` 在 `writeFile` 之后直接 `await syncMcpJson()`，因此就地编辑立即生效；监视器随后读到同一份内容时，`deepEqualJson` 判等会跳过重复的 apply，不产生二次挂载。
