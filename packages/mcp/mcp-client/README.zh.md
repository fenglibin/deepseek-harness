...
本包的配置不含任何密钥。HTTP 服务器的凭据由挂载方通过 `ctx.provide('mcpAuthSink', …)` 提供：建立每一代传输之前，监督器向该解析器索取一次 `Authorization` 值，因此刷新后的令牌无需重启即生效。没有解析器时，传输只带上配置里的 `headers`。

解析器返回"未授权"时，该服务器上报 `needs-auth` 并**停止重连**——服务器是可达的，缺的只是凭据，退避重试只会重复一次本来就正确的请求；返回"不可用"时上报 `failed`。解析器抛错被收容为"不可用"，因此另一个插件提供的凭据逻辑无法破坏它只作为旁观者的连接循环。
