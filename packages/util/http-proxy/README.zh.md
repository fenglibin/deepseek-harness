...
普通 `fetch()` 会走代理，任何使用 `globalThis.fetch` 的 SDK 也一样——MCP HTTP 传输与 pi-ai 提供方栈都使用它。应验证每个 SDK 的实际传输；例外见[已知限制与延期工作](#known-limitations-and-deferred-work)。
...
四个函数各对应调用方需要策略的一种方式：`installProxyFromEnvironment(env, report)` 解析并安装，返回一个 disposer；`proxyRouteFor(url)` 回答单次请求怎么发，并交回该答案所假定的传输；`proxyEnvironmentForChild()` 给出要应用到子进程环境的覆盖；`clearedProxyEnv()` 给出把每一个代理名字都删掉的那份覆盖。
...
该门禁看不进 SDK 内部，因此每个出网点都配有传输测试，通过假代理驱动实际传输并检查观察到的路由。每个新的出网点必须包含该传输测试。这些测试可发现调用点不变但依赖变更改变路由的情况。
...
loopback 始终被绕过——`localhost`、整个 `127.0.0.0/8` 段、`::1`、`0.0.0.0`，以及它们的 IPv4 映射写法。否则 Harness 自己的 Web UI、Connection 传输以及每一个本地测试服务器都会经由代理并形成回环。发布出去的绕过列表只包含读取环境的消费者能匹配的四个字面量条目；内部匹配器自行识别整个网段，因为列表条目无法表达一个范围。

`NO_PROXY` 的条目写的是主机名，它连同其下所有子域名一起匹配：`no_proxy=example.com` 也会放行 `api.example.com`。前缀 `.` 或 `*.` 可以写，含义相同。条目可带 `:port`，`*` 则放行全部。带方括号与裸写的 IPv6 字面量都能匹配——裸写的 `::1` **不会**被读成主机 `:` 端口 `1`，而 undici 自带的匹配器正是这样出错的，这也是解析结果中同时携带 `::1` 与 `[::1]` 的原因。CIDR 不参与匹配：操作系统的绕过列表常含 `10.0.0.0/8`，必须改写成后缀形式。
...
| 文件 | 承载 |
|---|---|
| [`src/policy.ts`](src/policy.ts) | 解析与绕过匹配；诊断只点名变量，从不带出它的值。不引入任何传输实现，因此在没有 undici 的环境中仍可加载。 |
| [`src/install.ts`](src/install.ts) | 全局 dispatcher、生效策略记录、路由与子进程环境。动态引入 undici。 |
| [`src/index.ts`](src/index.ts) | 本包的对外接口：四个函数与一个类型。 |
...
无，因为这个进程级传输策略不注册任何提示词、schema 或结果文本。
...
- **不支持自定义证书颁发机构**——做 TLS（传输层安全）拦截的企业代理需要在启动前为进程设置 `NODE_EXTRA_CA_CERTS`，本包既不设置也不校验它。
...
- **worker 线程不自动继承策略**——每个 worker 有自己的 `globalThis`，因而也有自己的 dispatcher；在此安装触及不到它。本仓库随附的两个 worker（workflow 引擎与 PTC 运行时）都执行模型编写的程序，因此都刻意不安装：它们绝不能拿到可能带有凭据的代理 URL。需要该策略的 worker 必须被显式交给一份并自行安装。
...
- **防回归门禁只看源码，看不到依赖内部**——`verify-no-bare-dispatcher` 解析 `packages/*/*/src` 与 `apps/*/src`；测试、脚本以及第三方 SDK 的内部都在其之外。
