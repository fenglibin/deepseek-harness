 ...
 *
 * See .agents/notes/implemented/testing/2026-06-19-acp-snapshot-tests.zh.md.
 *
...
/**
 * Poll `check` until it stops throwing, then rethrow its LAST diagnostic when the
 * timeout expires.
 *
 * `vi.waitFor` cannot express that: it drops the callback's error whenever the
 * timeout lands while a poll is still in flight and rejects with its own
 * `Timed out in waitFor!`. A poll reading a session log off disk takes longer
 * than the timeout on a loaded machine, so the scenario's actionable diagnostic
 * — which session lacked which record — was replaced by a generic message under
 * exactly the load a failing CI run has. Capturing the diagnostic and asserting
 * on the captured value keeps the message the caller matched against.
 * @param check - condition that throws the scenario diagnostic while unsatisfied.
 * @param timeoutMs - total budget before the last captured diagnostic is rethrown.
 */
async function waitForPersisted(check: () => Promise<void> | void, timeoutMs: number): Promise<void> {
  ...
  for (;;) {
    ...
    if (Date.now() >= deadline) {
      throw lastDiagnostic instanceof Error
        ? lastDiagnostic
        : new Error(`snapshot-harness: wait failed within ${timeoutMs}ms: ${String(lastDiagnostic)}`)
    }
    await setTimeout(WAIT_POLL_INTERVAL_MS)
  }
}
...
async function waitForPersistedTurnStart(
  ...
  await waitForPersisted(async () => {
    ...
    try {
      ...
      // the wait retries every callback throw, so capture the validation
