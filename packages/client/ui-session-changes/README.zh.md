...
列表上方还有两个按记录判定的动作：逐行的「查看变更」在全屏视图中以左右两栏对照展示该文件在会话中的累积变更，条带的「全部撤销」把本会话的改动从磁盘移除。
...
行上的「查看变更」打开一个**全屏视图**，以左右两栏对照该文件在本会话中的累积变更：左栏是会话开始前的内容，右栏是会话结束后的内容。条带头部的「全部撤销」把本会话的改动从磁盘移除，保留其它来源的改动。
...
这两个控件**逐行判定**：只有宿主确实记录了该路径修订的行才显示它们。变更列表读的是可以从完整日志重建的 `changedFiles` 投影，而修订记录活在宿主自己的存储里，两者的生命周期不同，因此列表知道的路径未必是可查看的路径。没有记录的行不显示控件，而不是给出一个点开必然失败的入口。条带的「全部撤销」按同一事实判定：记录集合不覆盖任何列出的路径时它不出现，否则会承诺一次「已撤销 0 个文件」。
...
- **查看与撤销需要宿主记录** — 这两个控件由宿主 `list` 动词报告的集合逐行决定；列表里的路径若没有对应记录（例如改动来自另一个会话，或该会话的记录超过存储上限）就不显示控件。
