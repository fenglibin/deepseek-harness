...
| [`src/invariant.ts`](src/invariant.ts) | 不变式伴生入口：本包不发布运行时检查的理由 |
...
| [`src/client/locales.ts`](src/client/locales.ts) | 全部可见与无障碍字符串的中文字典 |
...
- [ui-settings-general](../ui-settings-general/README.zh.md)——渲染导航并挂载该分区的设置外壳，也是本页导航图标的归属处。
...
无；该包既不组装也不发送提供方请求，因此不进入任何提供方请求前缀。
...
- **没有批量或分组操作**：归档集合按归档时间倒序整体渲染，因此没有批量取消归档、没有按 Workspace 分组，也没有排序切换。
...
**运行时不变式：** 伴生入口 [`src/invariant.ts`](src/invariant.ts) 只注册包归属，不发布检查；本包只注册一个本地化 `settings.section` 贡献及其 locale namespace，不发出 Cordis 事件，也不持有跨插件可变关系。
