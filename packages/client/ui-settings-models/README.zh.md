...
`dsh-client-ui-settings-models` 是 dsh Web 客户端的 Models 设置页面：用户可以配置 API 密钥（以只写方式存入 profile 的凭据引用之下）、编辑每个提供方的模型列表，并手工声明自定义 pi-ai 路由；页面以提供方行展示，一次只打开一张编辑卡片，而这张卡片以弹窗形式覆盖在分区之上。该页面把提供方目录、设置文档与凭据描述合并为一个共享快照，因此行的状态在三个方面始终一致。它还会带首次运行的用户走两个有序弹窗——版本化内测声明，以及按条件显示的官方 DeepSeek 凭据步骤。
...
从设置导航打开 Models 页面，即可看到每个已配置的提供方都有一行。点击**编辑**会把该行的编辑卡片以弹窗形式打开在分区之上，而不是在行内展开，因此列表始终是列表，卡片无论在哪里打开都保持同一宽度。其配置键未在任何位置配置的整分节提供方会自行打开同一个弹窗，但仅限首次运行姿态——页面上还没有任何提供方能够处理请求——且仅持续到用户关闭它为止。一次只打开一张卡片，因此关掉其中一张绝不会丢弃另一张里的草稿。
...
单个**添加模型**按钮打开一个对话框，两条获得提供方的路径都在其中。从其提供方 `<select>` 中挑选一条已安装路由——那些尚未配置且拥有 settings 地址的休眠目录行，裸挂载的 `llm-pi-ai` 在任何路由存在之前就能从其完整的已安装 catalog 中提供它们——会在对话框内展开该路由的普通提供方编辑卡片，与已保存行的弹窗所打开的编辑器是同一张。键入 API 地址并点击**继续**则展开手工声明的创建卡片，并以去除首尾空白后的该地址作为其 base URL；该卡片会索要唯一的 **Provider ID**、端点、协议与至少一个可唯一识别的模型，因为没有东西能为它们兜底。**返回**可从任一张卡片回到选择步骤；没有未配置的行可添加时，选择步骤会说明适配器目录中的提供方都已添加；而已展开的卡片从全部合并行中解析自己那一行，因此草稿进行中的刷新即使配置了该路由也不会拆掉卡片。**获取可用模型**通过 `llm/discoverModels` Remote 查询表单显示的端点，因此新增提供方一次即可完成，而非先保存再返回；回复打开的是选择器而非直接写入，只有点击**添加所选**才会写入。选择器打开时只勾选该提供方已配置的模型；提供方尚无任何行时，则勾选发现到的每一个模型。该选择器按模型 ID 或提供方给出的名称即时筛选，这正是长列表可用的原因；筛选只缩小显示的行，不缩小已勾选的集合，因此在不同筛选条件下勾选的模型会累积，直到点击**添加所选**。只有用户层单独携带某行时，该行才可删除（删除会恢复组合基线），其确认对话框会指名该提供方。
...
本分区为仓库外分发的插件声明两个席位，类型定义在 [`src/client/slot-contract.ts`](src/client/slot-contract.ts) 并从 `./client` 导出。`settings.models.provider-card`（keyed）渲染在展示该行卡片的那个界面上——已保存行由**编辑**打开的弹窗、首次运行姿态自行打开的同一个弹窗、以及添加对话框为所选路由展开的卡片——以 `entryKey = settingsNs` 分发，owner props 携带该行的 `ConfigurableProviderView`、其 configured 状态与已确认的 api-key 凭据状态，因此以某适配器家族的 namespace 注册一次即可收到该家族的全部卡片，含手工声明的路由；手工声明的草稿卡尚无目录行，保存之前不分发。某行的弹窗已打开时，席位分发到弹窗而不是行上，因此一张卡片绝不会收到两次。`settings.models.footer`（list）渲染在行列表与新增控件之后。注册方通过 `ctx.slots.inject` 激活，并以 type-only import 引入本包 `/client` 入口；没有注册方时两个席位均不渲染任何内容。
