...
单个**添加模型**按钮打开一个对话框，两条获得提供方的路径都在其中。从其提供方 `<select>` 中挑选一条已安装路由——那些尚未配置且拥有 settings 地址的休眠目录行，裸挂载的 `llm-pi-ai` 在任何路由存在之前就能从其完整的已安装 catalog 中提供它们——会在对话框内展开该路由的普通提供方编辑卡片，与已保存行展开的编辑器是同一张。键入 API 地址并点击**使用此地址**则展开手工声明的创建卡片，并以去除首尾空白后的该地址作为其 base URL；该卡片会索要唯一的 **Provider ID**、端点、协议与至少一个可唯一识别的模型，因为没有东西能为它们兜底。**返回**可从任一张卡片回到选择步骤；没有未配置的行可添加时，选择步骤会说明适配器目录中的提供方都已添加；而已展开的卡片从全部合并行中解析自己那一行，因此草稿进行中的刷新即使配置了该路由也不会拆掉卡片。**获取可用模型**通过 `llm/discoverModels` Remote 查询表单显示的端点，因此新增提供方一次即可完成，而非先保存再返回；回复打开的是选择器而非直接写入，只有点击**添加所选**才会写入。选择器打开时只勾选该提供方已配置的模型；提供方尚无任何行时，则勾选发现到的每一个模型。该选择器按模型 ID 或提供方给出的名称即时筛选，这正是长列表可用的原因；筛选只缩小显示的行，不缩小已勾选的集合，因此在不同筛选条件下勾选的模型会累积，直到点击**添加所选**。只有用户层单独携带某行时，该行才可删除（删除会恢复组合基线），其确认对话框会指名该提供方。
