/**
 * Image-understanding route preference: one vision provider/model picked from
 * the live model catalog and stored in its own settings namespace. The catalog
 * supplies the choices, and a stored route the catalog no longer advertises
 * stays listed so the user can still drop it. A browser whose settings scope
 * runs in memory mode never writes, so the card renders read-only instead of
 * claiming a save that the Host document will not hold.
 */
...
/** Namespace of the Host-owned image-understanding route. */
export const IMAGE_UNDERSTANDING_SETTINGS_NS = 'image-understanding'

/** Stored route; both fields empty means the user chose no describer. */
...
export interface ImageUnderstandingRoute {
...
export interface ImageUnderstandingCandidate extends ImageUnderstandingRoute {
...
/** State rendered by the image-understanding card. */
export interface ImageUnderstandingState {
  ...
  candidates: readonly ImageUnderstandingCandidate[]
...
type ImageUnderstandingDraft =
  | { readonly kind: 'route'; readonly route: ImageUnderstandingRoute }
  | { readonly kind: 'unset' }
 ...
 */
export function imageUnderstandingKey(route: ImageUnderstandingRoute): string {
...
export class ImageUnderstandingModelStore {
  /** The snapshot the card renders from (uSES-safe store). */
  readonly store: SnapshotStore<ImageUnderstandingState>

  private groups: readonly ModelProviderGroup[] = []
  private catalogStatus: ImageUnderstandingState['catalogStatus'] = 'idle'
  private draft: ImageUnderstandingDraft | undefined
  ...
  /**
   * @param scope - bound `image-understanding` settings scope.
   * @param ctx - the page plugin's context, whose `remote.session` namespace
   * answers the Host model catalog.
   */
  constructor(
    private readonly scope: SettingsScope<ImageUnderstandingSettings>,
  ...
  /**
   * Stage the absence of a route, so the Host discovers the first vision model
   * again. Shares {@link select}'s guards.
   */
  clear(): void {
    const snapshot = this.scope.getSnapshot()
    if (this.disposed || snapshot.status !== 'ready' || !snapshot.writable || this.saving) return
  ...
  private retained(): ImageUnderstandingRoute[] {
    const routes: ImageUnderstandingRoute[] = []
  ...
  private candidates(): readonly ImageUnderstandingCandidate[] {
    return imageUnderstandingCandidates(this.groups, this.retained())
  }

  private storedRoute(): ImageUnderstandingRoute {
  ...
  private storedKey(): string | undefined {
    ...
    return route.provider === '' ? undefined : imageUnderstandingKey(route)
  }

  private selectedKey(): string | undefined {
    const draft = this.draft
    if (draft === undefined) return this.storedKey()
    return draft.kind === 'route' ? imageUnderstandingKey(draft.route) : undefined
  }

  private projection(): ImageUnderstandingState {
    const snapshot = this.scope.getSnapshot()
