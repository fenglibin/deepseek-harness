...
`dsh-client-ui-settings-models` is the Models settings page of the dsh web client: users configure API keys (stored write-only under the profile's credential reference), edit each provider's model list, and hand-declare custom pi-ai routes, with provider rows and one editor card at a time, that card opening as a dialog over the section. The page joins the provider directory, the settings document, and the credential descriptions into one shared snapshot, so a row's state stays consistent across all three. It also walks first-run users through two ordered dialogs — a versioned internal-testing notice and the conditional official-DeepSeek credential step.
...
Open the Models page from the Settings navigation to see every configured provider as a row. **Edit** opens that row's editor card as a dialog over the section rather than expanding the row, so the list stays a list and the card keeps one width wherever it opens. A whole-section provider whose key is not configured anywhere opens that same dialog by itself, but only in the first-run posture — no provider on the page can serve requests yet — and only until the user closes it. One card is open at a time, so closing one never discards a draft in another.
...
One **Add model** button opens a dialog carrying both ways to gain a provider. Picking an installed route from its provider `<select>` — the dormant-directory rows that are not yet configured and have a settings address, which a bare-mounted `llm-pi-ai` supplies from its whole installed catalog before any route exists — expands the ordinary provider editor card for that route inside the dialog, the same editor a saved row's dialog opens. Typing an API base URL and pressing **Continue** expands the hand-declared create card seeded with that trimmed URL as its base URL; that card asks for a unique **Provider ID**, an endpoint, a protocol, and at least one uniquely-identified model, because nothing can default those. **Back** returns from either card to the chooser, which reports that every directory provider is already configured when no unconfigured row is left to add, and a card already open resolves its row from every joined row, so a refresh that configures that route mid-draft does not tear the card down. **Fetch available models** asks the `llm/discoverModels` Remote about the endpoint the form shows, so adding a provider is one pass instead of save-then-return; the reply opens a picker rather than being written, and nothing is written until **Add selected**. The picker opens with only the models the provider already configures selected, and with every discovered model selected when the provider carries no rows yet. That picker filters as you type on the model id or on the name the provider disclosed, which is what makes a listing of hundreds usable; the filter narrows the rows shown, never the selection, so picks made under different filters accumulate until **Add selected**. A row is deletable only when the user layer alone carries it (removal restores the composition base), and its confirmation dialog names the provider.
...
The section declares two seats for plugins distributed outside this repository, typed in [`src/client/slot-contract.ts`](src/client/slot-contract.ts) and exported from `./client`. `settings.models.provider-card` (keyed) renders on whichever surface shows that row's card — the dialog **Edit** opens for a saved row, the same dialog when the first-run posture opens it by itself, and the card the add dialog expands for a picked route — dispatched with `entryKey = settingsNs` and owner props carrying the row's `ConfigurableProviderView`, its configured state, and its confirmed api-key credential state, so one registration under an adapter family's namespace receives every card of that family, hand-declared routes included; the hand-declared draft card has no directory row yet and dispatches nothing until saved. A row whose dialog is open dispatches the seat there rather than on the row, so one card never receives it twice. `settings.models.footer` (list) renders after the rows and the add controls. A registrant activates through `ctx.slots.inject` with a type-only import of this package's `/client` entry; without registrants both seats render nothing.
