...
The resident composer survives no-Session and Session transitions. The no-Session state keeps the same composer surface mounted but inert while the Workspace picker connects a blank Session. A Session whose input shell is unreachable (its scope is mid-build or mid-teardown) renders that same inert surface instead of crashing the composer bar out of its slot, and the rebuilt scope re-runs the entry's inject; a shell whose scope was disposed owns no bindable editor, so the bar binds none. The surface is a shell-owned Lexical editor: reference chips are atomic decorator nodes carrying the owner's serialization identity (submission expands them through the owner codec), claimed slash commands stay styled leading text, folder text references carry the folder glyph as an icon prefix, inline images are zero-width decorator chips rendering their thumbnail with an inline remove control, and the draft's clipboard projection is mirrored into the per-Session Conversation store. Queue operations address exact queue occurrences through the scoped `ctx.conversation` service; queue previews render sent text through the shared inline reference projection from `ui-primitives` (wire session forms fold to their label), while an edit exposes the literal sent text. Busy Enter behavior is stored in the Host-backed `ui-conversation` settings namespace. Two dragged measurements are browser-local rather than settings: the transcript's content width and the composer's height, each a handle beside what it sizes, kept in `localStorage` under `dsh.conversation.contentWidth` and `dsh.conversation.composerHeight`. Both are clamped against the column that has to hold them without rewriting the stored preference, so a shrunken window clamps what is displayed and a grown one restores what the user dragged; the composer's handle also takes arrow keys and resets to the default on double-click.
...
While a normal composer is running, its primary pointer action remains Stop when the draft is empty or input is unavailable. Actionable text or inline images switch the same seat to Queue Send; clearing or successfully submitting the draft restores Stop. The busy-Enter setting continues to select the Queue or Steer keyboard action. Continuable subagents keep separate Send and Stop actions ([decision](../../../.agents/notes/implemented/bug-fix/2026-08-20-running-draft-primary-send.md)).
