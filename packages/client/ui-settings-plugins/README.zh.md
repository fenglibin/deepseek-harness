...
打开设置中的「插件」分区并选择**插件配置**标签页，即可编辑本部署所组装的宿主平面插件。卡片依次为 shell 执行器（`bash`）、agent 循环的工具调用并行度（`agent-loop`）、subagent 模型选择（`subagent-model-selection`）、DeepSeek 搜索提供方（`web-search-deepseek`）以及交付纪律（`delivery`）。

### 字段说明与帮助入口

每个字段的标签旁有一个问号图标，鼠标悬停时显示该字段的详细说明：它控制什么、取值如何影响行为、以及与其他字段的关系。问号是标签内的悬停亲和元素而非按钮，因此不会为当前字段增加第二个 Tab 停靠点。

交付纪律卡片另外提供一个**帮助**链接，点击后弹出参考对话框，内容覆盖：三档分级的判定条件与各自含义、每档的执行流程与必需记录、各级会产出的交付物、实现验证按三类输入执行的四道检查、复核轮次的放行范围、`covers` 注解的三种键名与写法示例，以及配置项的作用范围。该参考不内联在卡片里，因为卡片是表单——已经了解策略的用户不应为了到达字段而滚过整篇分级说明。
