...
交付纪律卡片另外提供一个**帮助**链接，点击后弹出参考对话框，内容覆盖：三档分级的判定条件与各自含义、每档的执行流程与必需记录、各级会产出的交付物、实现验证按三类输入执行的四道检查、复核轮次的放行范围、`covers` 注解的三种键名与写法示例，以及配置项的作用范围。该参考不内联在卡片里，因为卡片是表单——已经了解策略的用户不应为了到达字段而滚过整篇分级说明。链接与卡片描述文案同行，紧随该句之后：它展开的正是那句话，因此读起来属于同一行，而卡片标题行本身是一个展开/收起按钮，链接作为其兄弟节点而非子节点，以免出现按钮嵌套。

### 交付纪律卡片的控件

交付纪律共有 11 个字段，按两列栅格排布：枚举、开关与数字阈值各占一列，多行控件（分级规则文本框与验收命令选择器）整行占满，窄视口回落为单列。其它卡片的字段较少且不共用该栅格，因此保持各自的纵向布局。

**分级规则**改为一整段可编辑文本（`gradingPrompt`），以多行控件渲染并整行占满。这段文本直接作为一次独立分级模型调用的系统提示词，因此调整「什么算 L2」不需要改代码。原先的三个信号词表已移除：忽略大小写的子串匹配无法区分语义，任何词表都会同时漏判与误判。字段说明讲清了这段文本的作用、可以补充哪些团队约定，以及唯一不受它影响的判据是长度阈值（超过即直接为 L2，不再调用模型）。留空会被 Host 拒绝，因为空提示词会让每次判定都失败并回退到 L1。

**验收命令**不再是手写命令文本，而是从「提示词命令」设置页已配置的命令中勾选。控件分成两块：上方是**已选命令的执行顺序**列表，每条显示序号并支持拖动调整、方向键按钮上下移动与单条移除；下方是**候选命令**的勾选清单，勾选即追加到末尾，不重排用户已经排好的顺序。之所以区分两块，是因为数组顺序就是门禁要求模型依次执行的顺序，已选的命令必须能看见自己的位置，而未选的命令不该混在那个顺序里。

拖动与方向键共用控制器里的同一个移动动作（`moveVerificationCommand`），因此两条交互路径不会各自演变出不同的顺序语义；方向键不是可选装饰，而是仓库对交互控件的键盘可达性要求。已选但当前不再提供的命令仍然渲染在已选列表中（标注为已不在命令列表中），以便用户看见并移除，而不是让它悄悄留在下一次保存里。该命名空间未提供服务或没有命令时，卡片展示引导用户先到「提示词命令」页添加的空态，且不阻塞其它字段保存。
