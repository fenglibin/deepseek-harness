...
describe('rankByName', () => {
  ...
  it('matches the display title as a second key and keeps the stronger alignment', () => {
    const items = [
      { name: 'goal', title: '目标' },
      { name: 'plan', title: '计划' },
    ...
    // 标题前缀命中与名称前缀命中同权。
    expect(names(rankByName([{ name: 'xplan', title: 'plan' }, { name: 'plant' }], 'pla'))).toEqual(['xplan', 'plant'])
  })
