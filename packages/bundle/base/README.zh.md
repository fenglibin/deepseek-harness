...
默认文件编辑使用 `read`、`write` 和 `edit`。与它们重叠的 `str_replace_editor` 仍可显式启用：要将它加入基于本核心的 profile，请在 profile、home 或逐次调用 patch 中以 `insert` 形式添加以下条目：
...
只写 `disabled: false` 的 patch 需要已有配置项才能生效，无法创建配置项。
