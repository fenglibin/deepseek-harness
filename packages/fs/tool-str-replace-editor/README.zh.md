...
查看操作返回带行号文本或浅层目录列表。调用会提供文件位置，创建/替换调用还会向展示层提供 diff 卡片。修改操作返回简洁确认。长查看结果保留前缀并追加截断提示。结果的值是对象：修改操作携带 `path`、`before`、`after` 与 `operation`，查看操作只携带 `path` 与 `text`，`render` 把该值投影为上面这些文本——因此模型可见输出不变，而 `dsh-session-file-revisions` 能从 `tools/post-execute` 读到基线，使该工具的改动与 `write` / `edit` 一样可查看、可撤销。修改操作报告的是工具自己读到的 `before`，所以即使后端为超大覆写拒绝缓冲基准，捕获到的基线仍是完整的。
