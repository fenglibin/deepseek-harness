...
挂载策略后，`write` 只能在会话已观察到的版本上覆盖现有文件，`edit` 只能在这样的版本上编辑；自观察以来发生变化（包括缺失）的文件以 `FS_STALE_VERSION` 失败。未见目标不构成模型的义务：工具在当次调用内自行补读、记录该次观察并重新分发意图槽位，因此它从不作为模型可见的失败出现。缺失也会被记录：读取缺失文件会把它标记为确认缺失，因此随后的 `write` 可以通过防护创建流程重新创建它。会话恢复后不携带任何已观察状态，因此下一次防护变更会触发一次工具内补读。

### 失败与恢复

缺少先前观测时，意图槽位以代码 `FS_NOT_OBSERVED` 拒绝——但该拒绝由 [`dsh-tool-fs`](../tool-fs/README.zh.md) 与 [`dsh-tool-str-replace-editor`](../tool-str-replace-editor/README.zh.md) 在当次调用内结算：它们读取目标（这正是策略要求的观察）并重新分发槽位，于是变更紧接着成功，模型看不到那次拒绝。编辑被观测为缺失的目标仍以 `FS_NOT_FOUND` 失败。真正陈旧的观察以 `FS_STALE_VERSION` 失败，工具会重新读取目标并把当前内容附在失败消息里，省掉模型的一轮发现往返；目标无法重新读取时则保留纯恢复指令——先重新读取文件再重试。在外部删除的文件上遵循该恢复指令会记录缺失，因此下一次防护写入可以重新创建它，而不会覆盖并发创建者。
...
该插件不添加提示词或 schema。没有先前观测时，意图槽位会拒绝编辑并以 `FS_NOT_OBSERVED` 呈现，拒绝覆盖并以 `createIfAbsent` 门控；但工具在当次调用内结算它——读取目标（正是策略要求的观察）并重新分发槽位，因此模型看到的是成功而非拒绝。编辑被观测为缺失的目标仍返回 `FS_NOT_FOUND`。正向观测陈旧时，带防护的变更会传播由提供方拥有的 `FS_STALE_VERSION` 错误。[`dsh-tool-fs`](../tool-fs/README.zh.md) 拥有面向模型的错误包装：此时它在该次调用中重新读取目标并返回文件的当前内容；目标无法重新读取时则为消息追加恢复指令（`— re-read the file, then retry`），两种情况都保留错误码。外部删除目标后，遵循陈旧恢复指令会记录缺失：下一次带防护的写入可以通过 `createIfAbsent` 重新创建该目标，而提供方会以原子方式保留任何并发创建者写入的文件。
...
- **没有 agent（智能体）会话的参与者绝无法满足策略**：它们的编辑会抛出 `FS_NOT_OBSERVED`，写入总会解析为 `createIfAbsent`，因此非 agent 调用方无法通过门禁覆盖现有文件。`dsh-tool-fs` 与 `dsh-tool-str-replace-editor` 的补读结算同样要求一个可推导的所有者，因此该限制对经由工具的调用方同样成立。
- **直接 `ctx.fs` 读取不会发出 `fs/observed`**：在 `read` 工具之外读取的文件仍未观察，直到工具读取该文件（或经由工具的变更自行补读）。
