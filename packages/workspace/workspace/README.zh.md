...
删除会话是一项独立操作，它的后半部分属于本包：session controller 丢弃 durable 日志之后，`removeSession(sessionId)` 丢弃 registry 持有的每个引用——把该 id 从每个 Workspace 实体上解绑、清空内存中的 header 与 session-path 缓存，并把该 id 从归档集合中移除——全部发生在 registry 的单操作队列内。它不删除日志，并且刻意不校验该会话是否已知：它运行在日志已经消失之后，此时 `sessionPersistence.list()` 不再报告该 id，一次已知性校验反而会拒绝它本要执行的清理。解绑与缓存清理总会执行，而持久写入只在该 id 仍在归档集合中时发生，因此没有留下任何引用的会话会直接返回而不写入。
...
- **移除绝不删除数据**——移除项目会保留其文件夹、文件与会话历史；这些会话变成 Ungrouped。删除会话是一项独立操作，而文件夹移除是本包不提供的功能（参见[决策记录](../../../.agents/notes/implemented/feature/2026-07-27-workspace-registration-deletion.zh.md)）。
- **只有带记录目录的会话才能加入**——只有记录中带有可解析为项目路径的目录的会话才属于项目；没有目录的会话保持 Ungrouped，来自其他目录的会话无法移入。
- **外部变更延迟可见**——如果另一进程删除或损坏目录，项目只能在下次刷新或重启后反映出来。
- **归档是显示过滤器，而非所有权**——被隐藏的 Session 保留其历史与在 Workspace 行中的槽位，`unarchiveSession` 会把它恢复到该槽位保留的位置。
