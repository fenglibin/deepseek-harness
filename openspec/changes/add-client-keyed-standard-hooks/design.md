...
**把 `ResourceProtocolMap` 放在 `resources` 包**：不采用。协议属主（如工作区文件）需要声明自己的成员，若合并点位于资源实现包，属主就必须依赖该实现包，形成反向依赖。
...
**把 keyedHooks 合成与资源包拆成两个变更**：不采用。`resources` 包是 keyed hooks 的第一个真实消费者，拆开后类型合成的正确性缺少端到端证据。
