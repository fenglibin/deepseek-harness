...
flowchart LR
  ...
  plugin_dsh_base_response_language["response-language<br/>@deepseek-ai/dsh-response-language"]
  cfg --> plugin_dsh_base_response_language
...
| `response-language` | `@deepseek-ai/dsh-response-language` |
...
维护模式：hybrid: the patch row list is parsed from its `cordis.yml`; app package expansion is curated from package source。
