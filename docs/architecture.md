...
A tool result that carries images asks the mounted `image-understanding` service for a description when the target route declares text-only input, and the description lands in the same `tool/result` event.
