...
### 3.2 决策 B：在 `agent/pre-step` 做描述，而不是在网关，也不是在请求出口

网关不合适，因为它只负责准入消息，从描述到请求的关联仍要在更后面发生。请求出口不合适，因为那里的内容是深度冻结且未落盘的。`agent/pre-step` 是一个有文档记载的扩展点，loop 随后会把它的 decision 消息追加进会话日志，因此监听方既能产出描述，又能让描述被「承载消息本身」的同一套机制记录下来。`compaction-basic` 通过该事件改写消息，`plan-mode` 通过该事件追加持久状态，这个模式已经确立。
...
配置承载 `provider`、`model`、指令文本、输出上限、超时，以及理解调用所用的请求图片策略。`provider` 与 `model` 留空表示自动选择：挑第一条 `inputModalities` 含 `image` 的已注册模型，并把选择记入日志。配置出来的路由在首次使用时校验，必须声明图片输入能力，因此把纯文本路由配在这里会快速失败。调用本身复用请求图片阶梯，用压缩后的图片做描述，然后以新的 `image-understanding` purpose 通过 `ctx.llm` 流式发一次请求并取首个文本块，与压缩摘要器驱动辅助调用的方式完全一致。

### 4.3 `agent/pre-step` 上的 Consumer

该 Consumer 仅在 agent 声明的路由缺少图片输入时才工作。它遍历已认领消息中的图片块，用会话内缓存逐个解析尚未描述的附件，使每条附件只花一次视觉调用，然后返回一个 enter decision，携带附上描述后的消息副本。失败、超时与取消都不改动消息，只记一条警告，绝不阻断步骤。loop 会追加这些消息，描述因此随消息一起进入日志。
...
工具结果可以包含图片，例如截图工具，而它们不经过 inbox，因此 `agent/pre-step` 看不到。要覆盖它们需要为工具结果内容提供一个投影点，本设计有意不纳入。
...
`agent/pre-step` 看到的是 agent 声明的路由，而 `agent/request` 解析的才是实际派发的路由；改写路由的监听方因此可能与「是否描述」的判断不一致，此时要么产出一份用不上的描述，要么缺一份需要的描述而只能留占位文本。首步之前多出的几秒延迟是否需要一条非阻塞的输入框提示，属于产品取舍；本设计假定理解进行期间显示一条简短的本地化提示。
