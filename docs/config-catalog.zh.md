...
英文源文件由源代码（`scripts/gen-config-catalog.ts`）生成，并通过 `pnpm run verify-config-catalog`（`doc-sync` 的一部分）验证新鲜度；本中文文件作为对英文源的评审翻译。声明块使用 `ts config-catalog` 围栏（doc-typecheck 会跳过它，因为单独引用导入项的声明无法独立编译）。英文生成器还会将运行时 schemastery schema 与粘贴的声明进行交叉核对——每个经 schema 验证的键（包括嵌套键）都必须能在声明的配置类型中找到——因此，粘贴内容无法隐藏加载器接受的字段。
...
来源：[`packages/llm/image-understanding/src/index.ts:73`](../packages/llm/image-understanding/src/index.ts)
...
- `@deepseek-ai/dsh-mcp-manager`（[`packages/mcp/mcp-manager/src/index.ts`](../packages/mcp/mcp-manager/src/index.ts)）
