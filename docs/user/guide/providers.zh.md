...
![模型页：DeepSeek 卡片，及其下方的添加模型按钮](providers-models-page.zh.png)

密钥是只写的。保存后，页面只会收到脱敏描述符，永远不会收到明文密钥。密钥存储在 `$DSH_HOME/.credentials.yaml` 中，settings 只保留它的凭据引用。

## 添加提供方

选择**添加模型**。打开的对话框两条路都支持：选取 Anthropic 或 OpenAI 等已安装提供方，或输入目录未提供的提供方的 API 地址。已安装目录会为所选提供方提供端点、协议和模型列表，只需再输入其 API 密钥。

使用原生认证的提供方需要各自的原生凭据。Bedrock、Vertex、Azure 和 Codex 分别使用 AWS 凭据与区域、ADC 项目、`api-version` 和 OAuth；只填写 API 密钥字段无法完成配置。

### 按 API 地址添加提供方

对于公司网关、自建服务器或已安装目录中不存在的提供方，输入其 API 地址。需要提供小写 Provider ID、API 协议、凭据和至少一个模型；API 地址即基础 URL。
...
在**模型目录**中选择**获取可用模型**，可查询表单当前显示的基础 URL 和凭据。选择器默认勾选该提供方已配置的模型，因此重新选择会保留已有模型并追加新勾选的模型；尚无模型的提供方则默认勾选发现的全部模型。选择候选项只会更新草稿；保存前不会存储提供方。目录提供方使用已安装目录，不发起网络请求。
