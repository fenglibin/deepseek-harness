...
英文源文件由系统**生成**，并通过 `pnpm run verify-tool-catalog`（`doc-sync`（文档同步门禁）的一部分）验证新鲜度；本中文文件作为对英文源的评审翻译。与 Cordis 目录（纯源码 AST 处理）不同，英文生成器会在真实上下文中**启动**每个工具插件并读取 `ctx.tools.schemas()`，因为工具 schema 无法通过静态分析完全确定，例如运行时展开的枚举、拼接的描述、由配置决定的名称以及使用原始 JSON Schema 的 MCP 工具。完整性守卫会 glob 匹配 `packages/*/tool-*`；如果生成器的启动 manifest（元数据清单）遗漏任何包，检查就会失败，因此新工具不会在无人察觉的情况下缺少文档。参见[工具 schema 目录 Agent Note](../.agents/notes/implemented/process/2026-07-02-tool-schema-catalog.zh.md)。
...
| `@deepseek-ai/dsh-mcp-resources` | `list_mcp_resource_templates`, `list_mcp_resources`, `read_mcp_resource` | `ctx.tools`、`ctx.mcpResources` | `tool/call`、`tool/result` | - | 三个共享资源工具随作用域内的首个提供方出现、随最后一个卸载而移除；它们按 `server` 参数分派，不属于任何单个服务器，因此不受 mcp-client 的工具白名单影响。 |
...
针对当前交付任务记录一份 OpenSpec 变更产物，不改变其阶段，写入 openspec/changes/<change_id>/proposal.md（原因与内容）、design.md（技术决策）、tasks.md（勾选清单）或 specs/<capability>/spec.md（增量需求，每个至少含一个 `#### Scenario:` 与 SHALL/MUST 表述）。change_id 必须是动词开头的 kebab-case（add-、update-、remove-、refactor-）。任务在到达 specified 之前必须至少记录一条 spec。四件套以覆盖方式写入，因为 OpenSpec 会结构化解析 tasks.md 与增量 spec。
...
```json
{
  ...
  "properties": {
    ...
    "change_id": {
      "type": "string",
      "description": "Verb-led kebab-case OpenSpec change id."
    },
    "kind": {
      "type": "string",
      "enum": [
      ...
      "description": "Which change artifact to write."
    },
    "capability": {
      "type": "string",
      "description": "Kebab-case capability directory; required when kind is spec."
    },
    "text": {
      "type": "string",
      "description": "Non-empty content of the artifact."
    }
  },
  "required": [
    "task_id",
...
针对当前交付任务记录实施清单，整体替换此前记录过的清单。每一项为 { content, phase, done }：简短描述、所属生命周期阶段（created/designed/specified/implemented/verified/accepted）以及是否完成。该清单驱动任务展示中的分阶段进度，并在任务到达 implemented 前与 openspec tasks.md 交叉核对，因此需与该文件保持一致。
...
清单项跨模型边界传入，由服务侧的严格解码器裁定：内容重复、阶段非法或清单为空都会被拒绝。
